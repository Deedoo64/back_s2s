...
//===============================================================
// POST : Route pour ajouter une liste d'entries dans une List existante
//===============================================================
router.post("/entries", async (req, res) => {
  ...
  try {
    const { list, entryField } = await getListAndEntriesByListType(listId);

    console.log("List and entries found:", list, entries);
    ...
    for (const entry of entries) {
      ...
      if (list.type === "shopping") {
        newEntry = new ShoppingEntry({
          ...entry,
          _id: new mongoose.Types.ObjectId(),
        });
      } else if (list.type === "todo") {
        newEntry = new TaskEntry({
          ...entry,
          _id: new mongoose.Types.ObjectId(),
        });
      ...
      } else if (list.type === "tracking") {
        newEntry = new TrackingEntry({
          ...entry,
          _id: new mongoose.Types.ObjectId(),
        });
      }
      entryField.push(newEntry);
...
const getListAndEntriesByListType = async (listId) => {
  try {
    ...
    switch (list.type) {
      case "shopping":
        entries = list.shoppings; // Récupérer les articles de la liste de courses
        break;
      case "todo":
        entries = list.tasks; // Récupérer les tâches pour todo_list
        break;
      case "check":
        entries = list.checks; // Récupérer les items pour checklist
        break;
      case "tracking":
        entries = list.trackings; // Récupérer les entrées de suivi pour tracking list
        break;
      default:
